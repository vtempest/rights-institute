...
export function ParticleTextEffect({ words = [] }: ParticleTextEffectProps) {
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    // Calculate responsive font size to fill the space
    const baseFontSize = Math.min(canvas.width / 15, canvas.height / 4) // Smaller responsive font size
    const fontSize = Math.max(25, Math.min(40, baseFontSize)) // Clamp between 30-80px (smaller range)

    // Draw text
    offscreenCtx.fillStyle = "white"
    offscreenCtx.font = `bold ${fontSize}px Arial`  // Responsive font size
  ...
  const animate = () => {
    ...
    // Background with transparent motion blur
    ctx.fillStyle = "rgba(0, 0, 0, 0.05)"  // Much more transparent background
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      ...
      if (container) {
        ...
        canvas.height = (container.clientHeight || 500) * 0.6  // Reduce height to 60% of original
      } else {
        canvas.width = 1000
        canvas.height = 300  // Reduced from 500 to 300
      }
